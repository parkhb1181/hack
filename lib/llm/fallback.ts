 ...
 *
 * **밋밋하면 폴백을 쓰나 마나다.** LLM이 죽었을 때 사용자가 보는 게
 * 이거라서, 여기가 심심하면 화면 전체가 심심해진다. 상황을 놀리되
 * 사람을 깎아내리지 않는다 — 웃기는 것과 비웃는 건 다르다.
 */
const LINES: Record<string, [string[], string[]]> = {
  share: [
    ['이 정도면 대화가 아니라 너 혼자 브이로그야.', '스크롤 절반은 네 손가락이 만들었어.'],
    ['말풍선 개수로는 상대가 이겼어.', '상대가 말이 더 많아. 이거 흔한 그림 아닌데?'],
  ],
  length: [
    ['너는 편지 쓰고 상대는 답장 쓰네.', '한 줄이면 될 걸 너는 문단을 쓰더라.'],
    ['상대가 문단으로 답장하는데? 편지 쓰는 줄.', '길이로는 상대가 대하소설이야.'],
  ],
  question: [
    ['물음표는 죄다 네 몫이네. 무슨 면접 보냐?', '너 혼자 인터뷰 진행하고 있어.'],
    ['상대가 자꾸 캐묻는데? 무슨 취조야.', '물음표는 상대 쪽에서만 날아와.'],
  ],
  initiation: [
    ['첫 마디는 늘 네 담당이더라.', '대화 시작 버튼이 네 손에만 달렸어.'],
    ['먼저 말 거는 건 늘 상대야. 편하겠다.', '대화 시동은 상대가 걸어.'],
  ],
  emoji: [
    ['이모티콘 온도는 네 쪽이 한여름이야.', '표정은 네가 다 짓고 있어.'],
    ['이모티콘 온도는 상대 쪽이 한여름이야.', '표정은 상대가 다 짓고 있어.'],
  ],
  // 방향이 없는 축이라 양쪽에 같은 문구를 둔다
  cool: [
    ['어느 순간부터 갑자기 조용해졌는데?', '중간에 한 번 식었다 왔네.'],
    ['어느 순간부터 갑자기 조용해졌는데?', '중간에 한 번 식었다 왔네.'],
  ],
}
